...
Una nota di ambito: questa pratica è stata distillata su progetti grandi, con
...
- Le PRD e i piani di implementazione li scrive il tuo modello **più forte**.
  Un esecutore che scrive il piano per sé è dove la qualità crolla; mai lasciare
  che il tier operativo pianifichi per sé.
...
| Report di deviazione | durante l'esecuzione | rispondi alla più piccola domanda che sblocca, e solo quando la realtà diverge dal piano |
...
Tra un gate e l'altro non servi. Le skill portano il processo, così la tua
attenzione si spende solo nelle decisioni.
...
- **Le correzioni durevoli diventano lezioni.** Quando un umano dà una
  ...
  pagata cara), viene registrata come nota: un fatto per nota, tenuta sintetica,
  così le sessioni future la ereditano. È il sostituto gratuito della memoria
...
- [ ] Prerequisiti presenti: `git`, `gh` (autenticato), `node`/`npx`. Il flag
      `--check` li verifica.
