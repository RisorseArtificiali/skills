...
makes an agent's *process* predictable. This repo is ours, opinionated
and versioned.
...
workflow to your needs as you go. One principle does not scale with
...
3. **writing-prds**: phased PRD with falsifiable acceptance criteria;
   decompose into tasks.
4. **writing-plans** *(fork)*: one implementation plan per task. Fork
   addition: a *Deviation Protocol*; executors stop and report on
   deviation, never improvise a fix.
...
6. **adversarial-code-review**: mandatory gate before merging any branch;
   fresh reviewer subagents attacking from distinct lenses, then verifiers
   that must reproduce every finding. The PRD's acceptance criteria are the
   review contract.

Around the pipeline: **doubt-driven-development** *(fork)* for in-flight
second opinions, **review** for on-demand leveled reviews. **plan-walkthrough**
and **pr-walkthrough** review documents and PRs above the code level with
...
project life; `scripts/wire-machine.sh` automates the wiring.
...
parallel, each on its own task and in its own worktree. Our steady state is
...
Install the skill sets for Bob in `~/.bob/skills`:
...
The installer uses the [Skills CLI's Bob profile](https://github.com/vercel-labs/skills/blob/main/src/agents.ts)
(`--agent bob`), which targets Bob's dedicated directory instead of `~/.claude/skills`.

## Companion MCP servers

Three MCP servers complete the setup. None is required by the skills; all
are worth citing.
